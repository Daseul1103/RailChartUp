...
<meta charset="UTF-8">
	...
	$(document).ready( function() {
		...
		var form = document.mdCrowdRangeUpdateForm;
		
		$("#mdCrowdRangeUpdateForm").submit( function(event){
			var conUrl;
			...
			if(keyButton=="btnSubAll"){//전 범위 변경
				conUrl="/setting/mdCrowdRangeUpdate/all_"+loginId+".ajax";
			}else{//현재 화면만 변경
				conUrl="/setting/mdCrowdRangeUpdate/normal_"+loginId+".ajax";
			}
			//유효성 체크
				...
				$.ajax({
					...
					success: function(json){
						...
						if(json.msg=="" || typeof json.msg ==="undefined"){
							$("#content").load("/setting/mdCrowdRange.do");
						}else{
							alert(json.msg);
						}
					},
...
<body>
	<h3>월간(일별) 혼잡도 변위 설정 (단위 : KPA)</h3>
	<form id="mdCrowdRangeUpdateForm" name="mdCrowdRangeUpdateForm" method="post" enctype="multipart/form-data">
		<div class="setTbDiv">
			<table class='setTbList-nonbt'>
				<tr>
					...
					<td>
						<div class= inputSet>
							<input type="number" min="0" max="560" id ="mdWgtRlxDv" name="mdWgtRlxDv" value=${data.mdWgtRlxDv} maxlength="3" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' class="1" required>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class= inputSet>
							<input type="number" min="0" max="560" id ="mdWgtUsDv" name="mdWgtUsDv" value=${data.mdWgtUsDv} maxlength="3" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' class="3" required>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class= inputSet>
							<input type="number" min="0" max="560" id ="mdWgtCauDv" name="mdWgtCauDv" value=${data.mdWgtCauDv} maxlength="3" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' class="5" required>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class= inputSet>
							<input type="number" min="0" max="560" id ="mdWgtCwdDv" name="mdWgtCwdDv" value=${data.mdWgtCwdDv} maxlength="3" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' class="5" required>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						<div class= inputSet>
							<input type="number" min="0" max="1000" id ="mdWgtMargin" name="mdWgtMargin" value=${data.mdWgtMargin} maxlength="3" onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)' class="form-control" required>
						</div>
					</td>
				</tr>
			</table>
